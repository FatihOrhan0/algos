...
private: 
    ...
    vector<vector<int> > wordChecks;
public:
    bool checkInsertion(int word1, int word2, vector<string> & words) { 
        if (words[word2].size() != words[word1].size() + 1) return false;
        bool inserted = false;
        int i = 0;
        if (wordChecks[word1][word2] != -1) return wordChecks[word1][word2];
        if (wordChecks[word2][word1] != -1) return wordChecks[word2][word1];
        for (; i < words[word1].size(); i++) { 
            if (inserted) { 
                if (words[word1][i] != words[word2][i + 1]) {
                    wordChecks[word1][word2] = false;
                    return false;
                }
            }
            else { 
                if (words[word1][i] != words[word2][i]) {
                    inserted = true;
                    i--;
                }
            }
        }
        wordChecks[word1][word2] = true;
        return true;
    }
    
    int dpHelper(int i, int j, vector<string> & words) { 
        if (i == words.size()) return 0;
        if (dp.find(words[j]) != dp.end() && dp.find(words[j])->second.find(i) != dp.find(words[j])->second.end()) { return dp.find(words[j])->second.find(i)->second; }
        int ans = 0;
        while (true) { 
            if (i >= words.size()) break;
            if (words[i].size() > words[j].size() + 1) break;
            if (checkInsertion(j, i, words)) { 
                ans = std::max(1 + dpHelper(i + 1, i, words), ans);
            }
            i++;
        }
        if (dp.find(words[j]) != dp.end()) { 
            dp.find(words[j])->second.insert({i, ans});
        }
        else { 
            unordered_map<int, int> temp;
            temp.insert({i, ans});
            dp.insert({words[j], temp});
        }
        return ans;
    }
    int longestStrChain(vector<string> & words) {
        if (words.size() < 2) return words.size();
        this->wordChecks = vector<vector<int> >(words.size(), vector<int>(words.size(), -1));
        sort(words.begin(), words.end(), comp);
        int ans = 0;
        for (int i = 1; i < words.size(); i++) { 
            ans = std::max(1 + dpHelper(i, i - 1, words), ans);
        }
        return ans;
    }
};
